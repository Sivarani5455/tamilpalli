...
import type { Difficulty, Locale, NimishamExercise } from "@/types";
...
export function NimishamIndex({
  exercises,
  locale,
}: {
  exercises: NimishamExercise[];
  locale: Locale;
}) {
  ...
  const copy =
    locale === "ta"
      ? {
          ...
          points: "புள்ளிகள்",
          all: "அனைத்தும்",
          empty: "பொருந்தும் விளையாட்டு இல்லை.",
        }
      : locale === "fr"
        ? {
            search: "Rechercher...",
  ...
  const filteredExercises = useMemo(() => {
    ...
    return exercises.filter((exercise) => {
      ...
      const matchesQuery = `${exercise.title} ${exercise.description} ${exercise.slug} ${prompt}`.toLowerCase().includes(normalizedQuery);
      ...
      <main className="mx-auto max-w-[46rem]">
        ...
        <div className="space-y-3.5">
          ...
          {filteredExercises.map((exercise) => {
            ...
            const targetCount = exercise.words.filter((word) => word.isCorrect).length;
            const estimatedPoints = targetCount * 100;
